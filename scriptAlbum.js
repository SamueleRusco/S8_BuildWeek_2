  ...
  <div class="row col-md-8 justify-content-center">
    ...
    >
      ...
      >
        ...
        <div>
          <h3 class="fw-semibold">${jarray2[i].title}</h3>
          <p class="numAscolti">${jarray2[i].artist.name}</p>
        </div>
      </div>
      <div>
    
        <p class="numAscolti position-absolute end-50 ">${jarray2[i].rank}</p>
      

      <p class="numAscolti ms-5 position-absolute end-0 me-3">${
        jarray2[i].duration
      }</p></div></div>`;
        }
      }
